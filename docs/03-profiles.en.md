...
5. **Three rc-stage pitfalls**: use the `>=0.1.2-rc.1 <0.2.0` dependency line, always `await next()`, and client tests need the dsh runtime.
...
| Pitfall | Symptom | Fix |
|---|---|---|
| **rc.1 broken dependency chain** | `pnpm install` reports `@deepseek-ai/dsh-type-meta@0.0.1-rc.1` 404 | Several packages from the rc.1 era were never published; use the `>=0.1.2-rc.1 <0.2.0` line (npm published line `0.1.2-rc.1`) |
...
- **Q: Why does `pnpm install` fail with 404 errors?** The rc.1 dependency chain is broken. Use the `>=0.1.2-rc.1 <0.2.0` line in `package.json` (see Section 3.5).
