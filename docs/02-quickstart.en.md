...
> ⚠️ **npm published line vs GitHub tag** (verified 2026-09-06): npm's `latest` tag points at `0.1.2-rc.1` (`next` is also `0.1.2-rc.1`, `alpha` is `0.1.2-alpha.5`); the newest GitHub tag is `v0.1.3-alpha.1`, not yet published to npm. The commands below install the npm published line:
>
> ```bash
...
| plugin install 404 | **rc.1 dependency break**: use the `>=0.1.2-rc.1 <0.2.0` line (Ch.3 pitfall #1) |
