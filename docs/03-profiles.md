...
5. **rc 阶段三大坑**：依赖版本用 `>=0.1.2-rc.1 <0.2.0` 线、`next()` 必须 await、client 测试需要 dsh 运行时
...
| 坑 | 现象 | 解决 | 帖号 |
|---|---|---|---|
| **rc.1 依赖断裂** | `pnpm install` 报 `@deepseek-ai/dsh-type-meta@0.0.1-rc.1` 404 | 官方 rc.1 时代多个包从未发布；依赖线用 `>=0.1.2-rc.1 <0.2.0`（npm 已发布线 0.1.2-rc.1） | — |
